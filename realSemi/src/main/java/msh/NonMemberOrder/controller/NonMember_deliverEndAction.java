...
public class NonMember_deliverEndAction extends AbstractController {
		...
		if(!super.checkLogin(request)) {
			...
			if(!"admin".equals(userid) ) {
				...
				for(int i=0; i<fk_ordernumArr.length; i++) {
					...
					try {
					    n = pdao.NonMember_updateDeliverEnd(fk_ordernum);
				...
				if(n == fk_ordernumArr.length) {
	               ...
	               String message = "선택하신 제품들은 배송완료 처리했습니다.";
	               ...
	               super.setRedirect(false);
	               super.setViewPage("/WEB-INF/msg.jsp");
	            }//end of if(n == fk_ordernumArr.length) {-----------
	            
	            else {
	               String message = "선택하신 제품들은 배송완료로 실패되었습니다.";
	               String loc = "javascript:history.back();";
